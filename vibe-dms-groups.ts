...
import { ensureDMTables, pushToUsers, assertMessageAccess } from "./vibe-dms-core.ts";

export function registerDMGroupRoutes(app: Hono, registerMulti: RegisterMultiFn) {
  // ═════════════════════════════════════════════════════════════
  ...
  const handleCreateGroup = async (c: any) => {
    try {
      ...
      const groupName = String(name || "Nouveau groupe").trim().slice(0, 100) || "Nouveau groupe";
      const memberIds = Array.isArray(member_ids)
        ? Array.from(new Set(member_ids.map((x: any) => Number(x)).filter((n: number) => n && n !== userId))).slice(0, 49)
        : [];
      if (memberIds.length === 0) return c.json({ error: "Au moins un membre est requis." }, 400);
      ...
      for (const mid of allMembers) {
        ...
        if (mid !== userId) {
          ...
          `.catch(() => {});
          try {
            await pushRealtimeEvent(mid, "group_updated", { conversation_id: conversationId, action: "added" });
          } catch {}
        }
      }
      return c.json({ success: true, conversation_id: conversationId, name: groupName, member_count: allMembers.length }, 201);
    } catch (err: any) {
      console.error("[vibe-dms] createGroup error:", err);
      return c.json({ error: "Erreur création groupe." }, 500);
    }
  };
  registerMulti("post", ["/api/vibe/dms/groups", "/vibe/dms/groups", "/v1/dms/groups"], handleCreateGroup);
  ...
  const handleAddGroupMembers = async (c: any) => {
    try {
      ...
      const memberIds = Array.isArray(member_ids)
        ? Array.from(new Set(member_ids.map((x: any) => Number(x)).filter(Boolean))).slice(0, 49)
        : [];
      if (memberIds.length === 0) return c.json({ error: "Aucun membre à ajouter." }, 400);

      const sql = getDb();
      await ensureDMTables().catch(() => {});
      const convRows = await sql`SELECT is_group, group_name, created_by FROM dm_conversations WHERE id = ${groupId}::uuid LIMIT 1`;
      const conv = convRows[0];
      if (!conv || !conv.is_group) return c.json({ error: "Groupe introuvable." }, 404);
      if (Number(conv.created_by) !== userId) return c.json({ error: "Seul l'administrateur du groupe peut ajouter des membres." }, 403);
      ...
      for (const mid of memberIds) {
        ...
        if (res.length > 0) {
          ...
          try {
            await pushRealtimeEvent(mid, "group_updated", { conversation_id: groupId, action: "added" });
          } catch {}
        }
      }
      return c.json({ success: true, added, history_shared: true });
    } catch (err: any) {
      console.error("[vibe-dms] addGroupMembers error:", err);
      return c.json({ error: "Erreur ajout membres." }, 500);
    }
  };
  registerMulti("post", ["/api/vibe/dms/groups/:groupId/members", "/vibe/dms/groups/:groupId/members", "/v1/dms/groups/:groupId/members"], handleAddGroupMembers);
  ...
  const handleRemoveGroupMember = async (c: any) => {
    try {
      ...
      const sql = getDb();
      await ensureDMTables().catch(() => {});
      const convRows = await sql`SELECT created_by FROM dm_conversations WHERE id = ${groupId}::uuid LIMIT 1`;
      ...
      if (!isAdmin && targetId !== userId) return c.json({ error: "Seul l'administrateur peut retirer un membre." }, 403);
      if (targetId === Number(convRows[0].created_by)) return c.json({ error: "L'administrateur ne peut pas quitter son propre groupe." }, 400);

      await sql`DELETE FROM dm_group_members WHERE conversation_id = ${groupId}::uuid AND user_id = ${targetId}`;
      try {
        await pushRealtimeEvent(targetId, "group_updated", { conversation_id: groupId, action: "removed" });
      } catch {}
  ...
  registerMulti("delete", ["/api/vibe/dms/groups/:groupId/members/:userId", "/vibe/dms/groups/:groupId/members/:userId", "/v1/dms/groups/:groupId/members/:userId"], handleRemoveGroupMember);
  ...
  const handleGetGroup = async (c: any) => {
    try {
      ...
      const rows = await sql`
        ...
        LIMIT 1
      `;
      if (rows.length === 0) return c.json({ error: "Groupe introuvable." }, 404);
  ...
  registerMulti("get", ["/api/vibe/dms/groups/:groupId", "/vibe/dms/groups/:groupId", "/v1/dms/groups/:groupId"], handleGetGroup);
  ...
  const handleHideMessage = async (c: any) => {
    try {
      ...
      if (!access.ok) return c.json({ error: access.error }, access.code as any);
      await sql`
  ...
  registerMulti("post", ["/api/vibe/dms/messages/:messageId/hide", "/vibe/dms/messages/:messageId/hide", "/v1/dms/messages/:messageId/hide"], handleHideMessage);
  ...
  const handleSaveTranslation = async (c: any) => {
    try {
      ...
      const cleanLang = String(lang || "").trim().toLowerCase().slice(0, 12);
      const cleanText = String(text || "").trim().slice(0, 5000);
      if (!cleanLang || !cleanText) return c.json({ error: "Langue et texte requis." }, 400);
      const sql = getDb();
      await ensureDMTables().catch(() => {});
      const access = await assertMessageAccess(sql, userId, messageId);
      if (!access.ok) return c.json({ error: access.error }, access.code as any);

      const rows = await sql`SELECT translations FROM direct_messages WHERE id = ${messageId}::uuid LIMIT 1`;
      const current: Record<string, any> = (rows[0]?.translations && typeof rows[0].translations === "object") ? { ...rows[0].translations } : {};
      current[cleanLang] = { text: cleanText, detected: detected ? String(detected).slice(0, 12) : null, at: new Date().toISOString() };
      // Conserver au maximum les 3 langues les plus récentes
      const keys = Object.keys(current);
      if (keys.length > 3) {
        keys
          .sort((a, b) => Date.parse(current[a]?.at || 0) - Date.parse(current[b]?.at || 0))
  ...
  registerMulti("post", ["/api/vibe/dms/messages/:messageId/translations", "/vibe/dms/messages/:messageId/translations", "/v1/dms/messages/:messageId/translations"], handleSaveTranslation);
  ...
  const handleUpdateGroup = async (c: any) => {
    try {
      ...
      const sql = getDb();
      await ensureDMTables().catch(() => {});
      const convRows = await sql`SELECT created_by, group_name, group_avatar_url FROM dm_conversations WHERE id = ${groupId}::uuid AND is_group = TRUE LIMIT 1`;
      if (convRows.length === 0) return c.json({ error: "Groupe introuvable." }, 404);
      if (Number(convRows[0].created_by) !== userId) {
        return c.json({ error: "Seul l'administrateur du groupe peut le modifier." }, 403);
      }
      ...
      if (avatar_url !== undefined) {
        const url = avatar_url === null ? null : String(avatar_url).trim().slice(0, 1000) || null;
      ...
      await pushToUsers((memberRows as any[]).map((r) => Number(r.user_id)), "group_updated", { conversation_id: groupId, action: "updated" });
      const updated = await sql`SELECT group_name, group_avatar_url FROM dm_conversations WHERE id = ${groupId}::uuid LIMIT 1`;
      return c.json({ success: true, group: updated[0] || null });
    } catch (err: any) {
      console.error("[vibe-dms] updateGroup error:", err);
      return c.json({ error: "Erreur mise à jour groupe." }, 500);
    }
  };
  registerMulti("post", ["/api/vibe/dms/groups/:groupId", "/vibe/dms/groups/:groupId", "/v1/dms/groups/:groupId"], handleUpdateGroup);
  ...
  const handleDeleteGroup = async (c: any) => {
    try {
      ...
      if (convRows.length === 0) return c.json({ error: "Groupe introuvable." }, 404);
      if (Number(convRows[0].created_by) !== userId) {
        return c.json({ error: "Seul l'administrateur du groupe peut le supprimer." }, 403);
      }
      ...
      await pushToUsers(memberIds, "group_updated", { conversation_id: groupId, action: "deleted" });
      return c.json({ success: true, deleted: groupId });
    } catch (err: any) {
      console.error("[vibe-dms] deleteGroup error:", err);
      return c.json({ error: "Erreur suppression groupe." }, 500);
    }
  };
  registerMulti("delete", ["/api/vibe/dms/groups/:groupId", "/vibe/dms/groups/:groupId", "/v1/dms/groups/:groupId"], handleDeleteGroup);
  ...
  const handleTransferGroupAdmin = async (c: any) => {
    try {
      ...
      if (convRows.length === 0) return c.json({ error: "Groupe introuvable." }, 404);
      if (Number(convRows[0].created_by) !== userId) {
        return c.json({ error: "Seul l'administrateur peut transférer son rôle." }, 403);
      }
      ...
      `;
      if (targetMember.length === 0) return c.json({ error: "Ce membre ne fait pas partie du groupe." }, 404);
      ...
      const memberRows = await sql`SELECT user_id FROM dm_group_members WHERE conversation_id = ${groupId}::uuid`;
      await pushToUsers((memberRows as any[]).map((r) => Number(r.user_id)), "group_updated", { conversation_id: groupId, action: "admin_transferred" });
      return c.json({ success: true, new_admin: targetId });
    } catch (err: any) {
      console.error("[vibe-dms] transferGroupAdmin error:", err);
      return c.json({ error: "Erreur transfert administration." }, 500);
    }
  };
  registerMulti("post", ["/api/vibe/dms/groups/:groupId/members/:userId/transfer-admin", "/vibe/dms/groups/:groupId/members/:userId/transfer-admin", "/v1/dms/groups/:groupId/members/:userId/transfer-admin"], handleTransferGroupAdmin);
}
