...
export function registerVibeMAIRoutes(
  ...
  function detectTool(
    ...
    if (
      ...
      lower.startsWith("@summarize") ||
      lower.startsWith("/resumer") ||
      lower.startsWith("@resumer")
    ) {
      const t = cleanMsg
        .replace(/^[/@](summarize|resumer)\s*:?\s*/i, "")
        .trim();
      return { toolArgs: { target: t || "récents" }, toolToRun: "summarize" };
    }
    if (
  ...
  async function getUserAutoApprove(
    sql: any,
    userId: number
  ): Promise<boolean> {
    try {
    ...
    try {
      ...
      if (!toolToRun) {
        ...
        const resolveModel = (m: string) => {
          if (!m || m === "default" || m === "mai-1.5-light")
                ...
                "https://openrouter.ai/api/v1/chat/completions",
                {
                  body: JSON.stringify({
                    messages: [
                      ...
                      { content: message.trim(), role: "user" },
                    ],
                    model: candidate,
                  }),
                  headers: {
  ...
  const handleExecuteTool = async (c: any) => {
    try {
      ...
      const result = await MAIAgentFleet.executeTool(
        String(name),
        args,
        userId
      );
      const reply = result.success
        ? formatToolReply(String(name), result.result, "")
