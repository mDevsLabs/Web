...
function workView(work: CrossrefWork) {
  const authors = Array.isArray(work.author)
    ? work.author.slice(0, 8).map((item) => {
        const author = item as {
          family?: string;
          given?: string;
          ORCID?: string;
        };
  ...
  return {
    abstract: truncateText(work.abstract, 2000),
    authors,
...
export const searchPublications = tool({
  description:
    "Recherche des publications scientifiques et DOI à partir d'un sujet ou d'une citation.",
  execute: async ({ limit, query }) => {
    const url = crossrefUrl("/works", {
      "query.bibliographic": query,
      rows: String(limit),
      select:
        "DOI,title,type,published,container-title,author,URL,is-referenced-by-count",
    });
    const result = await fetchPublicJson<{ message?: CrossrefMessage }>(
      url.href,
      headers
    );
    ...
    return {
      publications: (message?.items ?? []).slice(0, limit).map(workView),
      source: sourceRef(url.href, "Crossref"),
      totalMatches: message?.["total-results"] ?? null,
    };
  },
...
export const getPublicationByDoi = tool({
  ...
  execute: async ({ doi }) => {
    ...
    const result = await fetchPublicJson<{ message?: CrossrefWork }>(
      url,
      headers
    );
    if (!result.ok) return { error: result.error };
    return {
      publication: workView(result.data.message ?? {}),
      source: sourceRef(`https://doi.org/${doi}`, "DOI / Crossref"),
    };
  },
  inputSchema: z.object({
    doi: z
...
export const searchAuthorPublications = tool({
  ...
  execute: async ({ author, limit }) => {
    ...
    return {
      author,
      publications: (message?.items ?? []).slice(0, limit).map(workView),
      source: sourceRef(url.href, "Crossref — auteur"),
      totalMatches: message?.["total-results"] ?? null,
    };
  },
  inputSchema: z.object({
...
export const searchJournals = tool({
  ...
  execute: async ({ limit, query }) => {
    const url = crossrefUrl("/journals", {
      query,
      rows: String(limit),
      select: "ISSN,title,publisher,subjects,counts",
    });
    ...
    const journals = (message?.items ?? []).slice(0, limit).map((journal) => ({
      ...
      works:
        (journal.counts as { "total-dois"?: number } | undefined)?.[
          "total-dois"
        ] ?? null,
    }));
    return {
      journals,
      source: sourceRef(url.href, "Crossref — revues"),
      totalMatches: message?.["total-results"] ?? null,
    };
  },
  inputSchema: z.object({
