...
export const qrCodeGenerator = tool({
  ...
  inputSchema: z.object({
    ...
    color: z
      ...
      .optional()
      .describe(
        "Couleur du QR code en code hexadécimal sans dièse ou avec (défaut: '000000')"
      ),
    data: z
      .string()
      .min(1)
      .max(2000)
      .describe(
        "Le texte, lien URL, contact vCard ou configuration Wi-Fi à encoder dans le QR Code"
      ),
    errorCorrection: z
      .enum(["L", "M", "Q", "H"])
      .optional()
      .describe(
        "Niveau de correction d'erreurs (L: 7%, M: 15%, Q: 25%, H: 30%, défaut: M)"
      ),
    format: z
      .enum(["svg", "png"])
      .optional()
      .describe(
        "Format de sortie souhaité : 'svg' (recommandé, vectoriel) ou 'png' (défaut: svg)"
      ),
    margin: z
      .number()
      .int()
