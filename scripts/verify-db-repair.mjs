...
console.log(
  "✅ 2. SELECT UserMemory avec filtres de portée (getGlobalMemories)"
);
...
console.log(
  "✅ 4. INSERT weekly_usage (user_id integer) — transaction annulée"
);
...
`;
console.log(
  `✅ 5. SELECT weekly_usage WHERE user_id = integer (handleGetUsage) : ${usage[0].used} tokens`
);

// 6. getPersistedTier : lecture users par id::text (users.id est integer)
const tier =
  await sql`SELECT id::text AS id, tier FROM users WHERE id::text = ${"1"}::text LIMIT 1`;
console.log(
  `✅ 6. getPersistedTier (users.id integer, lookup texte) : ${tier.length} ligne(s)`
);
