...
function checkUpstreamRemote() {
  ...
  if (!remotes.includes('upstream')) {
    console.log(`🌐 Configuration du remote 'upstream' vers ${UPSTREAM_URL}...`);
...
async function main() {
  ...
  if (fs.existsSync(STATE_FILE)) {
    try {
      ...
      if (resume === 'CANCEL') {
        console.log('Action annulée. Redémarrage du menu.');
        return main();
      }
      if (resume === 'y') {
        ...
        console.log(`✨ Reprise depuis le fichier ${startIndex + 1}.`);
      } else {
  ...
  while (i < unmergedFiles.length) {
    ...
    console.log(`\n======================================================`);
    console.log(`📄 Fichier ${i + 1} sur ${unmergedFiles.length} : \x1b[36m${file}\x1b[0m`);
    console.log(`   Statut Git : ${status} | Marqueurs de conflit : ${conflictCount}`);
    console.log(`   Stats : mAI (${stats.ours}) | Canary (${stats.theirs}) | Auto (${stats.auto})`);
    console.log(`======================================================`);

    if (file.includes('setting.json') || file.includes('setting.ts')) {
      console.log(`🔄 Règle automatique : Conservation de notre version (mAI) pour les traductions settings (${file})`);
    ...
    if (file.startsWith('locales/') && file.endsWith('.json')) {
      console.log(`🌍 Règle automatique : Fusion intelligente des traductions pour ${file} (Garde mAI pour existants, ajoute nouveautés Canary)`);
      try {
        const oursStr = runCmd(`git show :2:"${file}"`);
        const theirsStr = runCmd(`git show :3:"${file}"`);
        if (oursStr && theirsStr) {
          ...
          function mergeTranslations(o, t, currentPath = '') {
            if (typeof o !== 'object' || typeof t !== 'object' || o === null || t === null) {
              if (o !== undefined) {
                if (o !== t) changes.push({ path: currentPath, action: 'kept_ours', ours: o, theirs: t });
                return o;
              }
              changes.push({ path: currentPath, action: 'added_new', ours: undefined, theirs: t });
              return t;
            }
            ...
            for (const key of Object.keys(t)) {
              ...
              if (!(key in o)) {
                if (typeof t[key] === 'object' && t[key] !== null) {
                  result[key] = mergeTranslations({}, t[key], newPath);
                } else {
                  changes.push({ path: newPath, action: 'added_new', ours: undefined, theirs: t[key] });
                  result[key] = t[key];
                }
              } else {
                if (typeof o[key] === 'object' && typeof t[key] === 'object' && o[key] !== null && t[key] !== null) {
                  result[key] = mergeTranslations(o[key], t[key], newPath);
                } else {
                  if (o[key] !== t[key]) {
                    changes.push({ path: newPath, action: 'kept_ours', ours: o[key], theirs: t[key] });
                  }
                  result[key] = o[key];
                }
              }
          ...
          const mergedJson = mergeTranslations(oursJson, theirsJson);
          
          if (changes.length > 0) {
            console.log(`\n   🔍 Changements détectés :`);
            changes.forEach(c => {
              let color = '\x1b[36m';
              if (c.action === 'kept_ours') color = '\x1b[33m';
              else if (c.action === 'added_new') color = '\x1b[32m';
              console.log(`   ${color}[${c.action}]\x1b[0m ${c.path}`);
            });
            console.log();
          }

          fs.writeFileSync(file, JSON.stringify(mergedJson, null, 2) + '\n', 'utf-8');
          runCmd(`git add "${file}"`);
          console.log('✅ Fusion réussie pour ' + file);
          stats.auto++;
          i++;
          continue;
        }
      } catch (err) {
        console.log('❌ Erreur de fusion auto des locales, passage manuel. Erreur: ' + err.message);
      }
    }
    ...
    if (status === 'UD') {
      console.log('♻️  Règle automatique : Conservé (Modifié par canary, supprimé par nous)');
      runCmd(`git checkout --theirs "${file}"`);
      runCmd(`git add "${file}"`);
      stats.auto++;
      i++;
      ...
      console.log('♻️  Règle automatique : Conservé (Modifié par nous, supprimé par canary)');
      runCmd(`git checkout --ours "${file}"`);
      runCmd(`git add "${file}"`);
      stats.auto++;
      i++;
    ...
    if (autoDecision === 'ours') {
      console.log(`✅ Règle auto (${matchedDir}) : Version actuelle (mAI) conservée.`);
      runCmd(`git checkout --ours "${file}"`);
      runCmd(`git add "${file}"`);
      stats.auto++;
      i++;
      ...
      if (lostBranding.length > 0) {
        ...
        console.log(`   Mots clés détectés en local mais absents de Canary : ${lostBranding.join(', ')}`);
        const confirm = await askKey('❓ Voulez-vous VRAIMENT écraser avec la version Canary pour CE fichier ? (y/n) : ', ['y', 'n']);
        if (confirm !== 'y') {
          console.log('Décision automatique ignorée pour ce fichier, retour au mode manuel.');
          autoDecision = null; // Annule la décision auto pour ce fichier, il passe au prompt manuel
        }
      }
      
      if (autoDecision === 'theirs') {
        console.log(`✅ Règle auto (${matchedDir}) : Version entrante (canary) conservée.`);
        runCmd(`git checkout --theirs "${file}"`);
        runCmd(`git add "${file}"`);
        stats.auto++;
        i++;
    ...
    while (!validChoice) {
      console.log(`\nMenu d'options :`);
      console.log(`  [${keys.ours}] Garder Actuelle (mAI)`);
      console.log(`  [${keys.theirs}] Garder Entrante (canary)`);
      console.log(`  [${keys.skip}] Passer / Laisser les marqueurs`);
      if (dir) {
        console.log(`  [${keys.theirsDir}] Garder Entrante (canary) pour TOUT le dossier (${dir})`);
        console.log(`  [${keys.oursDir}] Garder Actuelle (mAI) pour TOUT le dossier (${dir})`);
      }
      if (topDir && topDir !== dir) {
        console.log(`  [${keys.theirsRoot}] Garder Entrante (canary) pour le dossier RACINE (${topDir})`);
        console.log(`  [${keys.oursRoot}] Garder Actuelle (mAI) pour le dossier RACINE (${topDir})`);
      }
      console.log(`  [${keys.theirsAll}] Garder Entrante (canary) pour TOUS les fichiers restants (Global)`);
      console.log(`  [${keys.oursAll}] Garder Actuelle (mAI) pour TOUS les fichiers restants (Global)`);
      console.log(`  [${keys.diff}] Voir le diff coloré`);
      console.log(`  [${keys.edit}] Ouvrir dans l'éditeur (VS Code/Cursor)`);
      console.log(`  [${keys.mergetool}] Lancer l'outil de fusion Git (mergetool)`);
      console.log(`  [${keys.abort}] Annuler TOTALEMENT la fusion (git merge --abort)`);
      if (history.length > 0) {
        console.log(`  [${keys.undo}] Annuler la dernière action (Undo)`);
      }
      console.log(`  [${keys.quit}] Quitter et sauvegarder l'état`);
      console.log(`  (Echap/Tab pour annuler un prompt en cours)`);
      ...
      if (choice === keys.abort) {
        ...
        if (confirm === 'y') {
          ...
          console.log('✅ Fusion annulée et état réinitialisé.');
          process.exit(0);
        }
        continue;
      }
  ...
  if (confirmCommit === 'y') {
    runCmdInteractive('git commit --no-edit');
    console.log('✅ Commit de fusion effectué !');
  } else {
    console.log('Vérifiez le statut avec `git status`. N\'oubliez pas de commiter !');
  }
}
