...
export async function ensureBooksTables(sql: any) {
  ...
  try {
    ...
    const missing =
      await sql`SELECT id FROM vibe_books WHERE join_code IS NULL LIMIT 500`.catch(
        () => []
      );
...
export function registerVibeBooksRoutes(
  app: Hono,
  ...
  const handleListBooks = async (c: any) => {
    try {
      ...
      const ownedRows =
        await sql`SELECT COUNT(*) AS n FROM vibe_books WHERE user_id = ${userId}`;
  ...
  registerMulti(
    "get",
    ["/api/vibe/books", "/vibe/books", "/v1/books"],
    handleListBooks
  );
  ...
  const handleCreateBook = async (c: any) => {
    try {
      ...
      const icon = String(body?.icon || "BookHeart")
        .trim()
        .slice(0, 40);
      ...
      if (!title)
        return c.json(
          { error: "Veuillez donner un titre à votre Livre." },
          400
        );
      if (title.length > 60)
        return c.json({ error: "Le titre est limité à 60 caractères." }, 400);
      ...
      if (Number(countRows[0]?.n || 0) >= MAX_BOOKS_PER_USER) {
        return c.json(
          {
            error: `Vous avez atteint la limite de ${MAX_BOOKS_PER_USER} Livres par compte.`,
          },
          403
        );
      }
      ...
      if (inserted.length === 0) {
        return c.json(
          { error: "Impossible de créer le Livre. Réessayez." },
          500
        );
      }
  ...
  const handleUpdateBook = async (c: any) => {
    try {
      ...
      const icon =
        body?.icon === undefined ? null : String(body.icon).trim().slice(0, 40);
      const isPublic =
      ...
      `;
      if (updated.length === 0)
        return c.json({ error: "Livre introuvable." }, 404);

      // Synchro du titre de la conversation Messages du Livre
      if (title)
        await renameBookConversation(sql, bookId, title).catch(() => {});

      return c.json({
        book: {
          ...updated[0],
          conversation_id: await getBookConversationId(sql, bookId),
          is_owner: true,
          role: "owner",
        },
        success: true,
  ...
  registerMulti(
    "delete",
    ["/api/vibe/books/:bookId", "/vibe/books/:bookId", "/v1/books/:bookId"],
    handleDeleteBook
  );
  ...
  const handleJoinBook = async (c: any) => {
    try {
      ...
      if (rows.length === 0) {
        return c.json(
          { error: "Aucun Livre ne correspond à ce code.", success: false },
          404
        );
      }
      ...
      const countRows =
        await sql`SELECT COUNT(*) AS n FROM vibe_book_members WHERE book_id = ${book.id}::uuid`;
  ...
  registerMulti(
    "post",
    ["/api/vibe/books/join", "/vibe/books/join", "/v1/books/join"],
    handleJoinBook
  );
  ...
  const handleGetBookMembers = async (c: any) => {
    try {
      ...
      return c.json({ members, success: true });
    } catch (err: any) {
  ...
  const handleLeaveBook = async (c: any) => {
    try {
      ...
      const rows =
        await sql`SELECT id, user_id FROM vibe_books WHERE id = ${bookId}::uuid LIMIT 1`;
      if (rows.length === 0)
        return c.json({ error: "Livre introuvable." }, 404);
        ...
        return c.json(
          {
            ...
            error:
              "Le créateur ne peut pas quitter son Livre. Supprimez-le à la place.",
            success: false,
          },
          403
        );
    ...
    try {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        ...
        try {
          ...
          `;
          return c.json({
            join_code: updated[0]?.join_code || code,
            success: true,
          });
        } catch {
          // collision de code : nouvelle tentative
  ...
  const handleToggleBookItem = async (c: any) => {
    try {
      ...
      const exists =
        await sql`SELECT 1 FROM posts WHERE id = ${postId}::uuid LIMIT 1`;
      if (exists.length === 0)
      ...
      if (already.length > 0) {
        ...
        return c.json({ saved: false, success: true });
      }
      await sql`
        ...
        `;
      return c.json({ saved: true, success: true });
    } catch (err: any) {
      console.error("[vibe-books] Toggle item error:", err);
      return c.json(
        { error: "Erreur lors de l'enregistrement dans le Livre." },
        500
      );
    }
  };
  ...
  const handleGetBookPosts = async (c: any) => {
    try {
      ...
      const posts = rows.map((r: any) => ({
        ...r,
        media_assets: byPost[String(r.id)] || [],
      }));
      ...
      return c.json({ book, members, posts, success: true });
    } catch (err: any) {
  ...
  const handleBooksForPost = async (c: any) => {
    try {
      ...
      return c.json({
        book_ids: rows.map((r: any) => String(r.book_id)),
        success: true,
      });
    } catch (err: any) {
      console.error("[vibe-books] Books for post error:", err);
      return c.json(
        { error: "Erreur lors de la vérification des Livres." },
        500
      );
    }
  };
  ...
  const handlePinBookPost = async (c: any) => {
    try {
      ...
      `;
      if (item.length === 0)
        return c.json({ error: "Cette Vibe n'est pas dans ce Livre." }, 404);
      ...
      if (pinned) {
        ...
        if (Number(countRows[0]?.n || 0) >= 3) {
          return c.json(
            {
              code: "PIN_LIMIT",
              error: "Maximum 3 Vibes épinglées par Livre.",
            },
            403
          );
        }
      ...
      return c.json({ pinned, success: true });
    } catch (err: any) {
  ...
  const handleKickBookMember = async (c: any) => {
    try {
      ...
      await ensureBooksTables(sql);
      const bookRows =
        await sql`SELECT user_id, title FROM vibe_books WHERE id = ${bookId}::uuid LIMIT 1`;
      if (bookRows.length === 0)
        return c.json({ error: "Livre introuvable." }, 404);
      if (Number(bookRows[0].user_id) !== userId) {
        return c.json(
          { error: "Seul le créateur du Livre peut exclure un membre." },
          403
        );
      }
      if (targetId === userId) {
        return c.json(
          { error: "Le créateur ne peut pas s'exclure de son propre Livre." },
          400
        );
      }
      ...
      `;
      if (del.length === 0)
        return c.json({ error: "Ce membre ne fait pas partie du Livre." }, 404);
  ...
  const handleTransferBookOwnership = async (c: any) => {
    try {
      ...
      if (Number(bookRows[0].user_id) !== userId) {
        return c.json(
          { error: "Seul le créateur du Livre peut transférer la propriété." },
          403
        );
      }
      ...
      `;
      if (updated.length === 0)
        return c.json({ error: "Transfert impossible." }, 409);
      ...
      await transferBookConversationOwnership(
        sql,
        bookId,
        userId,
        targetId
      ...
      return c.json({ new_owner: targetId, success: true });
    } catch (err: any) {
  ...
  const handleAddBookComment = async (c: any) => {
    try {
      ...
      if (!plain)
        return c.json(
          { error: "Le message de la discussion est requis." },
          400
        );
      ...
      if (replyToId) {
        ...
        if (replyRows.length === 0)
          return c.json(
            { error: "Le message auquel vous répondez est introuvable." },
            400
          );
      ...
      if (ownerId !== userId) {
        try {
          ...
          `;
          await pushRealtimeEvent(ownerId, "notification", {
            actor_id: userId,
            book_id: bookId,
            type: "book_join",
          });
      ...
      return c.json({ comment, success: true }, 201);
    } catch (err: any) {
  ...
  const handleDeleteBookComment = async (c: any) => {
    try {
      ...
      if (
        Number(rows[0].author_id) !== userId &&
        Number(rows[0].owner_id) !== userId
      ) {
        return c.json(
          {
            error:
              "Seuls l'auteur du commentaire ou le créateur du Livre peuvent le supprimer.",
          },
          403
        );
      }
  ...
  const handleSearchPublicBooks = async (c: any) => {
    try {
      ...
      const q = String(c.req.query("q") || "")
        .trim()
        .slice(0, 50);
      if (!q) return c.json({ books: [], success: true });
      ...
      const rows = await sql`
        ...
        LIMIT 20
      `;
      return c.json({ books: rows, success: true });
    } catch (err: any) {
  ...
  registerMulti(
    "get",
    ["/api/vibe/books/public", "/vibe/books/public", "/v1/books/public"],
