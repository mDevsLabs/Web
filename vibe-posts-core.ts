...
export async function publishDuePosts(): Promise<void> {
  ...
  try {
    ...
    console.warn(
      "[vibe-posts] publishDuePosts skipped:",
      (err as any)?.message
    );
  }
}
...
export async function attachQuotedPosts(posts: any[]) {
  ...
  const quoteIds = Array.from(
    new Set(posts.map((p) => p.quoted_post_id).filter(Boolean))
  );
  ...
  try {
    ...
    for (const m of mediaRows) {
      (mediaByPost[String(m.post_id)] ||= []).push({
        alt_text: m.alt_text,
    ...
    const byId = new Map(
      rows.map((r: any) => [
        String(r.id),
        { ...r, media_assets: mediaByPost[String(r.id)] || [] },
      ])
    );
    for (const p of posts) {
      if (p.quoted_post_id)
        p.quoted_post = byId.get(String(p.quoted_post_id)) || null;
    }
  } catch (err) {
...
export async function fetchPostMedia(posts: any[]) {
  ...
  try {
    ...
    const media =
      await sql`SELECT post_id, url, media_type, alt_text FROM media_assets WHERE post_id = ANY(${ids}::uuid[]) ORDER BY position ASC, created_at ASC`;
  ...
  try {
    ...
    if (currentUserId) {
      ...
      for (const v of votes)
        votesByPoll[String(v.poll_id)] = String(v.option_id);
    }
...
export async function attachCollaborators(posts: any[]) {
  ...
  try {
    ...
    console.warn(
      "[vibe-posts] attachCollaborators skipped:",
      (err as any)?.message
    );
  }
}
...
export async function attachPollsAndCollabs(
  posts: any[],
...
export const ensurePostColumns = async () => {
  ...
  try {
    ...
    console.warn(
      "[vibe-posts] ensurePostColumns skipped:",
      (err as any)?.message
    );
  }
};
...
export const ensureProfilePinnedPostsTable = async () => {
  ...
  try {
    ...
    console.warn(
      "[vibe-posts] ensureProfilePinnedPostsTable skipped:",
      (err as any)?.message
    );
  }
};
...
export async function syncPostBookRefs(
  sql: any,
  postId: string,
  bookIds: string[]
): Promise<void> {
  try {
    ...
    console.warn(
      "[vibe-posts] syncPostBookRefs skipped:",
      (err as any)?.message
    );
  }
}
...
export async function attachBookRefs(
  posts: any[],
  ...
  try {
    ...
    const bookIds = Array.from(
      new Set(refRows.map((r: any) => String(r.book_id)))
    );
    const books = await sql`
      SELECT b.id, b.title, b.icon, COALESCE(b.is_public, FALSE) AS is_public, b.user_id,
             ...
             ${viewerId === null ? sql`FALSE` : sql`(b.user_id = ${viewerId} OR EXISTS (SELECT 1 FROM vibe_book_members bm2 WHERE bm2.book_id = b.id AND bm2.user_id = ${viewerId}))`} AS viewer_is_member
      FROM vibe_books b
