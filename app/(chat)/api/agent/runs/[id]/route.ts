...
import { getAgentRunById, setAgentRunFeedback, updateAgentRunStatus } from "@/lib/db/agent-queries";

const feedbackSchema = z.object({
  useful: z.boolean().optional(),
  goalReached: z.boolean().optional(),
}).refine((value) => value.useful !== undefined || value.goalReached !== undefined);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const run = await setAgentRunFeedback({ id, userId: user.id || user.email, ...body.data });
  if (!run) return errorResponse("not_found");
  return Response.json({ useful: run.useful, goalReached: run.goalReached }, { headers: { "Cache-Control": "private, no-store" } });
}
