...
export default function AgentsClient() {
  const {
    ...
    mutate,
    isLoading,
  } = useSWR<{ agents: Agent[]; limit: number | null }>("/api/agents", fetcher);
  const agents = useMemo(() => agentsData?.agents ?? [], [agentsData]);
  // Quota d'agents du forfait : Plus 15 / Pro 25 / Max illimité (`null`).
  const agentLimit = agentsData?.limit ?? null;
  const isAtAgentLimit = agentLimit !== null && agents.length >= agentLimit;
  ...
  const [activeTab, setActiveTab] = useState<"agents" | "templates" | "stats">(
    "agents"
  );
  ...
  const [formColor, setFormColor] = useState(DEFAULT_AGENT_COLOR);
  ...
  // Filtres de l'onglet « Modèles » :.tags et .defaultModelId des modèles.
  ...
  const templateModelIds = useMemo(
    () => [...new Set(templates.map((t) => t.defaultModelId))].sort(),
    [templates]
  );
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return templates.filter((template) => {
      if (templateModel && template.defaultModelId !== templateModel) {
        return false;
      }
      if (templateTag && !(template.tags ?? []).includes(templateTag)) {
        return false;
      }
      if (!query) {
        return true;
  ...
  }, [templateModel, templateQuery, templateTag, templates]);
  ...
  const handleNewAgent = useCallback((template?: AgentTemplate) => {
    ...
    setFormColor(template?.color ?? DEFAULT_AGENT_COLOR);
  ...
  const handleEditAgent = (a: Agent) => {
    ...
    setFormColor(a.color || DEFAULT_AGENT_COLOR);
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setTemplateModel(e.target.value || null)}
                  value={templateModel ?? ""}
                >
                  <option value="">Tous les modèles</option>
                  {templateModelIds.map((modelId) => (
                    <option key={modelId} value={modelId}>
                      {modelId}
                    </option>
                  ))}
                </select>
                {(templateQuery || templateTag || templateModel) && (
                  <Button
                  ...
                  >
                    <div className="flex items-center justify-between w-full mb-2">
                      <div
                        ...
                        style={{
                          backgroundColor: t.color || DEFAULT_AGENT_COLOR,
                        }}
                      >
                        <AgentIcon icon={t.icon} size={16} variant="plain" />
                      </div>
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          ...
                          style={{
                            backgroundColor: a.color || DEFAULT_AGENT_COLOR,
                          }}
                        >
                          <AgentIcon icon={a.icon} size={20} variant="plain" />
                        </div>
                    ...
                    <div className="flex flex-col gap-2 pt-2 border-t border-border/40 text-[11px]">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          variant={
                            activeAgent?.id === a.id ? "secondary" : "outline"
                          }
                        >
      ...
      <Dialog onOpenChange={setIsEditorOpen} open={isEditorOpen}>
        <DialogContent className="max-w-5xl sm:max-w-4xl lg:max-w-6xl w-[95vw] max-h-[90vh] overflow-y-auto">
          {/* `pr-12` réserve l'espace du bouton de fermeture, positionné en
              `absolute top-4 right-4` par DialogContent : sans cela il
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 flex flex-col gap-4">
                ...
                <div className="space-y-2 rounded-xl border border-border/50 p-3 bg-muted/20">
                  <Label className="text-xs font-semibold">
                    Icône — affichée partout (sidebar, @, cartes, dropdown)
                  </Label>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        ...
                        style={{ backgroundColor: formColor }}
                      >
                        <AgentIcon icon={formIcon} size={20} variant="plain" />
                      </div>
                      <div className="min-w-0">
                        <h3 className="font-semibold text-sm text-foreground truncate flex items-center gap-1.5">
